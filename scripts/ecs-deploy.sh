...
TASK_ID=`aws ecs list-tasks -c $CLUSTER_NAME --desired-status RUNNING --family $TASK_DEFINITION_API | egrep "task" | tr "/" " " | tr "[" " " |  awk '{print $2}' | sed 's/"$//'`
...
aws ecs update-service -c $CLUSTER_NAME --service $SERVICE_NAME --task-definition $TASK_DEFINITION_API:${TASK_REVISION} --desired-count 1
